...
```{r message=FALSE}
 ...
 clean_student_profile<-na.omit(student_profile)
 
 # It is important to note that we removed the HAS_DISHWASHER column since it basically had nothing but NA values so we didn't include it in the new student_profile
 
 
 clean_student_profile
 
 # Summarizing new clean_student_profile data frame
 
 clean_overall_sum <- summary(clean_student_profile)
